...
/**
 * The device that you would like to communicate with (the
 * “server” of BLE). Contains both the current state of the
 * device (its properties, {@link Characteristic}s, etc.)
 * and functions for communication.
 */
public class Peripheral implements AutoCloseable {
    ...
    /**
     * Creates a connection to the device. If this method
     * returns {@code true} there has been successful
     * connection.
     *
     * <p>Note that {@link Peripheral}s allow only one
     * connection at a time. Operations that attempt to
     * communicate with a device will fail until it is
     * connected.
     *
     ...
     *
     * @return Whether the connection has been successfully
     * terminated
     */
    ...
    /**
     * Discovers all {@link Service}s for the device,
     * including their {@link Characteristic}s.
     *
    ...
    /**
     * Get the set of {@link Service}s we’ve discovered for
     * this device. This will be empty until
    ...
    /**
     * Sends a read request to the device. Returns either
     * {@code null} if the request was not accepted or the
     * response from the device.
     *
     * @param characteristic the {@link Characteristic} to
     *                       read data from
     * @return The response from the device, or {@code null}
    ...
    /**
     * Write some data to the {@link Characteristic}.
     * Returns {@code false} if the write couldn’t be sent
     * or (in the case of a write-with-response) if the
     * device returns an error.
     *
     * @param characteristic the {@link Characteristic}
     *                       to write data to
     * @param data the data to be written
     * @param withoutResponse whether the device should
     *                        respond
     * @return Whether the write is successful
     */
    public boolean writeValue(@NonNull Characteristic characteristic, byte[] data, boolean withoutResponse) {
        try (Arena tempArena = Arena.ofConfined()) {
    ...
    /**
     * Enables either notify or indicate (depending on
     * support) for the specified {@link Characteristic}.
     * A same {@link Characteristic} cannot be subscribed
     * more than once.
     *
     * @param characteristic the {@link Characteristic}
     *                       to subscribe
     * @param onDataReceived how the data should be
     *                       handled
     * @return Whether the subscription is successful
     */
    public boolean subscribe(@NonNull Characteristic characteristic, Consumer<byte[]> onDataReceived) {
        ...
        try {
            ...
            boolean result = BtleplugFfi.ble_peripheral_subscribe(ctxPtr, peripheralPtr, characteristic.charPtr, stub, MemorySegment.NULL);
    ...
    /**
     * Disables either notify or indicate (depending on
     * support) for the specified {@link Characteristic}.
     *
     * @param characteristic the {@link Characteristic}
     *                       to unsubscribe
    ...
    public boolean unsubscribe(@NonNull Characteristic characteristic) {
        boolean result = BtleplugFfi.ble_peripheral_unsubscribe(ctxPtr, peripheralPtr, characteristic.charPtr);
    ...
    /**
     * Sends a read descriptor request to the device.
     * Returns either {@code null} if the request was not
     * accepted or the response from the device.
     *
     * @param descriptor the {@link Descriptor} to read
     *                   data from
     * @return The response from the device, or {@code null}
     * if the request is not accepted
     */
    ...
    /**
     * Write some data to the {@link Descriptor}. Returns an
     * error if the write couldn’t be sent or (in the case
     * of a write-with-response) if the device returns an
     * error.
     *
     * @param descriptor the {@link Descriptor} to write
     *                   data to
     * @param data the data to be written
     * @return Whether the write is successful
     */
    ...
    /**
     * Release the {@link Peripheral}'s memory. Also closes
     * the connection if already connected.
     */
    @Override
    public void close() {
